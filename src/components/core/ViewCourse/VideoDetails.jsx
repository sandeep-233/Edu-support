...
const VideoDetails = () => {
    ...
    const {courseSectionData, courseEntireData, completedLeactures} = useSelector((state) => state.viewCourse);
    ...
    useEffect(() => {
        const setVideoSpecificDetails = async() => {
            if(!courseSectionData.lenght)
                return;
            if(!courseId && !sectionId && !subSectionId) {
                navigate("/dashboard/enrolled-courses");
            }
            else {
                const filterData = courseEntireData.filter(
            ...
            >
...

                {
                    videoEnded && (
                        <div>
                            {
                                !completedLeactures.includes(subSectionId) && (
                                    <IconBtn
                                     disabled={loading}
                                     onClick={() => handleLectureCompletion()}
                                     text={!loading ? "Mark As Comleted" : "Loading..."}
                                    />
                                )
                            }
